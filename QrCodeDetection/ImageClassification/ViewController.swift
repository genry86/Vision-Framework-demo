...
extension ViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(
        ...
        let detectBarcodesRequest = VNDetectBarcodesRequest(completionHandler: { request, error in
    ...
    func handleResult(_ request: VNRequest) {
        if let results = request.results as? [VNBarcodeObservation] {
            for result in results {
                if let payload = result.payloadStringValue, self.classTitle.text != payload {
                    print("QR-код найден: \(payload)")
                    self.classTitle.text = payload
                    return
                }
            }
        }
